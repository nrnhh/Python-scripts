...
async def export_to_excel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if last_action == 'specialty' and 'specialty_df' in context.user_data:
            ...
            # Burada bitirib əsas menyuya qayıdırıq
            await update.effective_chat.send_message(
        ...
        # Fərdi və TOP 10 üçün mürəkkəb formatlama (Ayrı Sheetlər və Pivot)
            ...
            with oracledb.connect(user=username, password=password, dsn=dsn) as conn:
                ...
                df_raw['Məlumat Tipi'] = df_raw['Məlumat Tipi'] + ' - ' + df_raw['Həkim Adı']
                df_raw['Ay_Adi'] = df_raw['Ay'].apply(lambda x: datetime.strptime(str(x), '%Y-%m').strftime('%B %Y'))
                ...
                for (tip, xidmet_novu), df_g in df_raw.groupby(['Məlumat Tipi', 'Ayaktan Sayı']):
                    # Pivotlama
                    if xidmet_novu == 0: # Yalnız Yatanlar üçün (Ayaktan 0 olanda)
                        pivot_df = df_g.pivot_table(index='Həkim Adı', columns='Ay_Adi', values='Yatan Sayı', aggfunc='sum', fill_value=0)
                        metric_label = 'Yatan Sayı'
                    else: # Yalnız Ayaktanlar üçün (Ayaktan > 0 olanda)
                        pivot_df = df_g.pivot_table(index='Həkim Adı', columns='Ay_Adi', values='Ayaktan Sayı', aggfunc='sum', fill_value=0)
                        metric_label = 'Ayaktan Sayı'
                    ...
                    total_row['Həkim Adı'] = 'TOPLAM'
                    pivot_df = pd.concat([pivot_df, total_row.set_index('TOPLAM', drop=True)], ignore_index=False)
                    
                    # Ayları düzgün sırayla almaq üçün
                    ...
                    # Sheet adını yarat
                    sheet_name = f"{tip[:20]} - {metric_label}"
                    ws = wb.create_sheet(sheet_name)
                    
                    # DataFrame-i sheetə yaz
                    rows = dataframe_to_rows(pivot_df.reset_index().rename(columns={'index': 'Həkim Adı'}), header=True, index=False)
                    for r_idx, row in enumerate(rows, 1):
                        ws.append(row)

                    # Formatlaşdırma
                    for row_cells in ws.iter_rows(min_row=1, max_row=ws.max_row, min_col=1, max_col=ws.max_column):
                        is_header = (row_cells[0].row == 1)
                        ...
                        for cell in row_cells:
                            ...
                            elif cell.column == ws.max_column: # Son sütun (CƏMİ)
                                cell.font = Font(bold=True)
...
# Qalan funksiyalar (Dəyişilməyib)
# ... (generate_graph, generate_specialty_graph, generate_single_graph, prompt_excel_export, select_specialty, ask_doctor, button_handler funksiyaları əvvəlki kimi qalır) ...
# YUKARIDAKI GENERATE_GRAPH VE BASHQA FUNKSIYALARDAKI LOGIC DUZELISHLERI QORUNUB
# YENI KODDA YALNIZ TEKRARLANMAMASI UCUN SILINDI. TAM KODU ASAGIDA BIRLESHDIRIRIK.

# --- BİRLƏŞDİRİLMİŞ SON KOD BLOKU ---

# **Qeyd:** Zəhmət olmasa, bu son kodu tam şəkildə kopyalayıb işlədin.
...
chat_id = '-1003177573983'
bot_token = "8386936045:AAE18mrqRK9SxgDjj0nbglv5B5sxH1Waaxo"
PASSIVE_DOCTOR_LINK = "[https://t.me/+pVPrA77Eq08xOWZi](https://t.me/+pVPrA77Eq08xOWZi)"
        ...
        # Fərdi və TOP 10 üçün mürəkkəb formatlama (Ayrı Sheetlər və Pivot)
            ...
            with oracledb.connect(user=username, password=password, dsn=dsn) as conn:
                ...
                df_raw.rename(columns={'TIP': 'Məlumat Tipi', 'HEKIM_ADI': 'Həkim Adı', 'AY': 'Ay', 
                                       'AYAKTAN': 'Ayaktan Sayı', 'YATAN': 'Yatan Sayı', 
                                       'YEAR_': 'İl', 'MONTH_': 'Ay_Reqem'}, inplace=True)
                # Unikal tip qruplarını yarat: Məlumat Tipi + Ayaktan/Yatan
                df_raw['Qrup_Adı'] = df_raw['Məlumat Tipi'] + ' - ' + df_raw['Ayaktan Sayı'].apply(lambda x: 'AYAKTAN' if x > 0 else 'YATAN')
                df_raw['Ay_Adi'] = df_raw['Ay'].apply(lambda x: datetime.strptime(str(x), '%Y-%m').strftime('%B %Y'))
                ...
                # Hər bir tip və xidmət növü üçün ayrı sheet yarat
                for qrup_adi, df_g in df_raw.groupby('Qrup_Adı'):
                    # Pivotlama
                    # Ayaktan və Yatanın hansı olduğunu tap
                    is_ayaktan = 'AYAKTAN' in qrup_adi
                    value_metric = 'Ayaktan Sayı' if is_ayaktan else 'Yatan Sayı'

                    pivot_df = df_g.pivot_table(index='Həkim Adı', columns='Ay_Adi', values=value_metric, aggfunc='sum', fill_value=0)
                    ...
                    # Sheet adını yarat
                    sheet_name = qrup_adi.replace(' - ', '_').replace(' ', '_')[:30]
                    ws = wb.create_sheet(sheet_name)
                    
                    # Başlıq sətrini yaz
                    ws.append(['Həkim Adı'] + list(pivot_df.columns))

                    # DataFrame-in qalan sətirlərini yaz
                    for r_idx, row in enumerate(dataframe_to_rows(pivot_df, header=False, index=True), 2):
                        ws.append(row)
...
async def generate_specialty_graph(update: Update, context: ContextTypes.DEFAULT_TYPE, stat_type: str):
    ...
    try:
        ...
        # category_names lüğətini burada təyin etməyə ehtiyac yoxdur, çünki xəta mesajında istifadə olunmayıb, sadəcə təminat üçün yoxlayırıq.
        await update.effective_chat.send_message(
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # --- Xüsusi düymələr ---
            ...
            await query.message.reply_text(
                "❌ Passiv Həkimlər siyahısına baxmaq üçün digər botun olduğu qrupa yönləndirirəm:",
            ...
            if context.user_data.get('last_action') == 'manual' or context.user_data.get('last_action') == 'manual_reselect_date':
                # Fərdi (manual) axtarış: İndi həkim kodunu soruşuruq və ya yenidən hesablama edirik.
