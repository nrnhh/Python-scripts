...
- Hər ay üçün ayrıca Excel sheet (Yanvar, Fevral, ...).
- Sütunlar: DEPO, Xeste sayi, Mebleg, Alis meblegi.
- Rəngli başlıq, auto-fit, filter, freeze panes, CƏM sətri.
- "Mebleg" və "Alis meblegi" üçün data bar vizualı.
"""
...
# Instant Client qovluğu
INSTANT_CLIENT_DIR =  r"C:\instant\instantclient_23_9"

# Tarix aralığı (bitis EXCLUSIVE). Məs: 2025-08-01 → 2025-11-01 (Avqust–Oktyabr)
START_DATE = date(2025, 1, 1)
END_DATE   = date(2025, 11, 1)

# Filtrlər
ODEME_DURUM = "*Ödenmis"
FORMTYPE_LIST = (2,)   # yalnız 2; 2,3,10 lazım olsa: (2,3,10)

# Çıxış faylı
OUTPUT_XLSX = "Stok_Cixis_Statistikasi_Depoya_Gore_Aylıq.xlsx"


# ================== SQL ==================
# Ay-ay (sheet başına) icra ediləcək sorğu
SQL = f"""
SELECT
            ...
            while cur_start < END_DATE:
                ...
                for r in range(nrows):
                    ...
                    for c, name in enumerate(df.columns):
                        ...
                        elif name in ("Mebleg", "Alis meblegi"):
                ...
                # Data bars (vizual)
                if nrows > 1:
                    if "Mebleg" in df.columns:
                        c = df.columns.get_loc("Mebleg")
                        ws.conditional_format(3, c, 2 + nrows, c, {"type": "data_bar"})
                    if "Alis meblegi" in df.columns:
                        c = df.columns.get_loc("Alis meblegi")
                        ws.conditional_format(3, c, 2 + nrows, c, {"type": "data_bar"})
