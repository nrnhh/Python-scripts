...
# Passiv Həkimlər üçün qrup linki
...
def get_main_menu():
    keyboard = [
        ...
        [InlineKeyboardButton("❌ Passiv Həkimlər", callback_data='menu_passive')], # Dəyişdirildi
...
async def top_10_doctors(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with oracledb.connect(user=username, password=password, dsn=dsn) as conn:
            ...
            # TOP 10 olduğu üçün burada tarix dəyişmə opsiyası yoxdur.
            await update.effective_chat.send_message(
...
async def export_to_excel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Bu hissə əsasən son nəticənin hansı kateqoriyaya aid olduğunu göstərir
...
async def generate_graph(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with oracledb.connect(user=username, password=password, dsn=dsn) as conn:
            ...
            # Fərdi (manual) rejimində buranı EXPORT_PROMPT-ə keçirir, TOP10/Specialty rejimində isə sadəcə növbəti addımı gözləyir.
            context.user_data['last_action'] = 'manual' if category == 'hekim' else 'top10_result' # 'top10_result' yeni bir tag
            await update.effective_chat.send_message(
                f"📊 {category_name} statistikasını Excel olaraq göndərilsin mi?\n"
                "Hə: Excel faylı göndəriləcək.\nYox: Ana menyuya qayıdacaqsınız.",
                reply_markup=get_export_prompt_buttons(context.user_data['last_action']), # Dəyişiklik burada
                reply_to_message_id=update.effective_message.message_id
            )
            return EXPORT_PROMPT
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if query.data == 'menu_stats':
            # 1. Fərdi Həkim: Default tarix (1 il) təyin edilir, istifadəçiyə soruşulmur.
            context.user_data.clear() # Bütün əvvəlki işi təmizlə
            context.user_data['kodlar'] = []
            context.user_data['adlar'] = []
            context.user_data['date_range'] = 365 # Default 1 il
            end = datetime.now()
            start = end - timedelta(days=365)
            context.user_data['start_date'] = start.strftime('%d.%m.%Y')
            context.user_data['end_date'] = end.strftime('%d.%m.%Y')
            context.user_data['last_action'] = 'manual'
            ...
            logger.info("Statistik Qrafiklər seçildi. Default tarix təyin edildi.")
            return ASK_DOCTOR
            ...
            logger.info("Tarixə görə Analiz seçildi (Qayıt).")
            return MAIN_MENU
            ...
            context.user_data['last_action'] = 'top10'
            logger.info(f"TOP 10 {context.user_data['top10_category']} seçildi, peşə seçim menyusuna yönləndirilir.")
            await query.message.reply_text(
        ...
        # --- Xüsusi düymələr ---
            ...
            logger.info("Passiv Həkimlər seçildi. Xarici linkə yönləndirildi.")
            return MAIN_MENU
            ...
            # 6. Tarix Aralığı Seçimi: Nəticəyə Keçid
            ...
            if context.user_data.get('last_action') == 'top10':
                ...
                await query.message.reply_text(
                    "⚠️ Əvvəlcə bir kateqoriya (TOP 10 və ya Peşə) seçin.",
                    reply_markup=get_return_main_menu_button(),
                    reply_to_message_id=query.message.message_id
                )
                return MAIN_MENU
        
        # --- Fərdi Axtarışdan Sonra Tarix Dəyişikliyi (YENİ MƏNTİQ) ---
            ...
            if context.user_data.get('last_action') == 'manual' and context.user_data.get('kodlar'):
                ...
                # Mərhələ SELECT_DATE_RANGE-ə qayıdır, lakin 'last_action' hələ də 'manual' qalır.
                return SELECT_DATE_RANGE 
            else:
                await query.message.reply_text(
        ...
        # Peşə statistikası geri qayıt düymələri
            ...
            await query.message.reply_text(
                ...
                reply_markup=get_continue_buttons(context.user_data['last_action']), # Dəyişiklik burada
                reply_to_message_id=query.message.message_id
            )
            return CONTINUE_DOCTOR

        elif query.data in stat_types:
            # Peşə Statistikasının Növü Seçimi: Qrafik və Nəticə
...
async def ask_doctor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Tarix məlumatlarını context-dən götür (default olaraq menu_stats-da təyin edilib)
    start_date = context.user_data.get('start_date')
    end_date = context.user_data.get('end_date')
    days = context.user_data.get('date_range', 365) # Yeni tarix aralığı seçildisə yenilənir
    
    # Əgər date_range-də yenidən gəlmişiksə, yeni tarixləri qeyd edək
    if context.user_data.get('last_action') == 'manual' and not context.user_data.get('kodlar'):
        # Bu, ilkin 'menu_stats' sorğusudur, tarix artıq təyin olunub
        pass 
    elif context.user_data.get('last_action') == 'manual' and not context.user_data.get('search_list'):
        # Bu, 'continue_yes' və ya yeni tarix seçimi sonrası təkrar axtarışdır.
        # Əgər biz bura SELECT_DATE_RANGE-dən qayıtmışıqsa, yenidən qrafik hazırlama mərhələsinə keçə bilərik (aşağıda).
        pass


    logger.info(f"Həkim axtarışı: {search_input}, Tarix aralığı: {start_date} - {end_date}, chat_id: {update.effective_chat.id}")

    # İstifadəçiyə tarix barədə təsdiqləyici mesaj göndərilir
    await update.effective_chat.send_message(
    ...
    try:
        with oracledb.connect(user=username, password=password, dsn=dsn) as conn:
            ...
            if len(df) == 1:
                ...
                await update.effective_chat.send_message(
                    "Başqa həkim əlavə etmək istəyirsiniz?\nNövbəti addım: Hə seçsəniz yeni həkim əlavə edə, Yox seçsəniz statistikaları görə bilərsiniz.",
                    reply_markup=get_continue_buttons(context.user_data['last_action']), # last_action 'manual' olacaq
                    reply_to_message_id=update.message.message_id
                )
